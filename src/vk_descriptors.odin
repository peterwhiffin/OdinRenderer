...
create_descriptor_layouts :: proc(ren: ^Renderer, res: ^Resources) {
	tex_bind: []vk.DescriptorSetLayoutBinding = {
		{
			...
			descriptorCount = u32(res.images.len),
			stageFlags = {.FRAGMENT},
		},
	...
	pick_bind: []vk.DescriptorSetLayoutBinding = {
		{
			binding = 0,
	...
	pick_ci: vk.DescriptorSetLayoutCreateInfo = {
		sType        = .DESCRIPTOR_SET_LAYOUT_CREATE_INFO,
		bindingCount = u32(len(pick_bind)),
		pBindings    = raw_data(pick_bind),
	}

	check(vk.CreateDescriptorSetLayout(ren.device, &pick_ci, nil, &ren.desc_layout_pick))
}
...
descriptor_set_create_tex :: proc(ren: ^Renderer, res: ^Resources) {
	...
	it := xar.iterator(&res.images)

	for img in xar.iterate_by_ptr(&it) {
...
update_pick_descriptors :: proc(ren: ^Renderer) {
	for set, i in ren.desc_set_pick {
		buffer_info: vk.DescriptorBufferInfo = {
			buffer = ren.picking_buffers[i].buff,
			offset = 0,
			range  = size_of(Picking_Data),
		}
...
descriptor_set_create_pick :: proc(ren: ^Renderer, res: ^Resources) {
	ren.desc_set_pick = make([]vk.DescriptorSet, FIF)
	layouts: [FIF]vk.DescriptorSetLayout

	for &l in layouts {
		l = ren.desc_layout_pick
	}
	...
	check(vk.AllocateDescriptorSets(ren.device, &dai, raw_data(ren.desc_set_pick)))
	update_pick_descriptors(ren)
}
